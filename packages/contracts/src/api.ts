...
export const API_ROUTES = Object.freeze({
  ...
  disable_key: {
    ...
    auth: "merchant_key",
    description:
      "Stops one of this merchant's keys working, from that instant, and touches no other key. Disabling a key that is already disabled changes nothing and answers the same way, keeping the instant it was first revoked at, so a retry after a dropped connection is safe. Two refusals are worth knowing before a screen is built on this. A key belonging to another merchant is answered exactly as a key that does not exist, so this call is not a way of counting somebody else's keys. And the key this call was made with cannot be disabled by it: that is one click between a merchant and a cabinet the gateway will not take, and the way back is a terminal they do not have. What that second refusal does not do is worth reading before anything is built on top of it. It is about the key on this call and nothing else, so a merchant holding two keys can disable either one with the other, including the one their cabinet is signed in with — the gateway does not know which key that is, and nothing here can tell it. A merchant left with no working key, or with a live sign-in against a key that no longer opens the door, is reachable through this call and is not refused by it. Rotating the key a cabinet itself holds is a separate act and is not built.",

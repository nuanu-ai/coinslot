...
проекта до первой продажи, — на странице [«Первая продажа»](/quickstart).
...
| `price` | сумма и валюта | обязательно | `{ amount: '5.00', currency: 'USD' }` |
...
выдачу. Оба видны агенту до покупки, а что происходит, когда они выходят,
разобрано в разделе [«Время вышло»](/orders).
...
и работают обе. Ответ хука сильнее цены в карточке: ответил — продажа идёт по
его цене; молчит — мы берём цену из карточки, и дальнейшее зависит от режима
выдачи ([«Что может пойти не так»](/failures)).

У асинхронного товара с хуком цена в карточке нужна ровно для одного —
показать агенту, во что примерно обойдётся покупка при выборе в каталоге.
Продажа такого товара идёт только по ответу хука: деньги здесь списываются в
момент покупки, поэтому молчащий хук покупку не начинает, и запасным
...
Адрес вы объявляете в карточке, а мы приходим туда HTTP-запросом в момент
покупки. Форма запроса и ответа ниже рабочая, до пилота она может измениться.
...
Поле `price_id` — идентификатор этого вопроса о цене. Тот же `price_id`
приходит потом вместе с заказом, и по нему вы связываете свой ответ с
продажей: под названную цену можно отложить остаток, а когда срок жизни цены
выйдет и заказ так и не появится — вернуть отложенное в продажу. Резервировать
никто не обязан: без резерва хук остаётся ответом на вопрос, а не
обязательством.
...
для системы и назначаем его мы, а что происходит по его истечении — в разделе
[«Время вышло»](/orders).
...
правило, по которому решается, нужен ли товару хук: товар, который может
кончиться, стоит выставлять с хуком, потому что без него мы продаём по цене в
...
мы и агент, причину читает человек, который потом разбирает случай. Разбор
самих отказов — на странице [«Заказы и режимы выдачи»](/orders); здесь —
словарь кодов.
...
хука `available: false` в ту же метрику попадает и без кода.
...
Теми же инструментами, которыми она заведена. Снятая карточка перестаёт быть
видимой в каталогах, а незакрытые заказы по ней доигрываются обычным
порядком.

...
//   node probe.mjs validate <url>   — официальная валидация листинга (без API-ключа)
...
if (cmd === "local") {
  ...
  const r = await fetch("https://api.cdp.coinbase.com/platform/v2/x402/validate", {
    ...
    body: JSON.stringify({ resource: arg }),
  });
  out(`validate ${arg} → HTTP ${r.status}`, await r.json().catch(() => ({})));
} else if (cmd === "bazaar") {
  ...
  console.log("usage: node probe.mjs local|validate <url>|bazaar <query>");
}
